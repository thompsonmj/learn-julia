...
# ╔═╡ 765d1360-ecb0-423a-8447-429ac975f96a
begin
	...
	using Symbolics
	#using MomentClosure # not yet a registered package 
	#using JuliaSim # cloud service
end

# ╔═╡ 6760d080-ee29-11eb-3477-f967a8e82800
md"""
What is [ModelingToolkit.jl (MTK)](https://mtk.sciml.ai/stable/)]?
"""

# ╔═╡ 529cb6ed-d4b1-4741-8be5-bc166ac82e70
md"""
It's the symbolic side of the SciML organization.

1. Symbolic modeling for all numerical simulation.

We have all these numerical tools like DiffEqFlux.jl and differential equations solvers. MTK is the symbolic part of that. You define your equation symbolically, and then it automatically handles it and turns it into things that are solvable.

It's a symbolic numeric optimizer. It takes your model and does some things on it. "We changed your model here; now it's more parallel. We changed your model there, and it's more stable to solve. We changed it here, and it's faster to solve."

MTK is an acausal modeling system
"""

# ╔═╡ 3fb7339f-5f25-4eb6-972f-9a6d50eaf89a
let
	# WORKING
	# 17:47
	# https://youtu.be/HEVOgSLBzWA?t=1067
	
	
	# t, σ, ρ, β = nothing, nothing, nothing, nothing
...
# ╔═╡ f23eea7e-fcd6-4a42-9d8a-d30963a23ec0
md"""
Mixed Continuous and Discrete Differential-Algebraic Equations
"""

# ╔═╡ 93c470d9-a9b8-4f42-97b7-da5c147ed51e
# let
# 	# NOT WORKING
# 	# 19:42
# 	# https://youtu.be/HEVOgSLBzWA?t=1182
	
# 	@parameters t a b c d f
# 	@variables x(t) y(t)
# 	δ = Differential(t)
...
# 		0 ~ f*y + sin(f*a)
# 	]
# 	de = ODESystem(eqs,t,[x,y,f],[a,b,c,d])
# end

# ╔═╡ 4a6c2dca-17a0-4ad4-a761-46057c9b0221
md"""
You don't have to "use MTK" to use MTK!
"""

# ╔═╡ d10db144-4f15-4fa1-8f42-433aebcca614
# begin
# 	# NOT WORKING
# 	# 20:42
# 	# https://youtu.be/HEVOgSLBzWA?t=1242
...
# 	fastprob = ODEProblem(sys,jac=true)
	
# 	# using BenchmarkTools
# 	@btime solve(prob)
# 	@btime solve(fastprob)
# end

# ╔═╡ a81e0a81-70ae-481b-bf03-684837f33fd3
md"""
Models can come from all sorts of domain-specific languages (DSLs)!
"""

# ╔═╡ d6c12c9d-3851-42ab-b905-069876cf337f
let
	# WORKING
	# 21:52
	# https://youtu.be/HEVOgSLBzWA?t=1312
...
# ╔═╡ af703a60-6b36-43ee-a785-a6e3c744493f
md"""
Once you have a model inside MTK, what can you do with it?
"""

# ╔═╡ 3ff63d78-d961-48e1-8a01-87caff93377d
let
	# 23:51
	# https://youtu.be/HEVOgSLBzWA?t=1431
	
	# Example source: 
	# https://github.com/SciML/ModelingToolkit.jl/blob/master/docs/src/tutorials/ode_modeling.md 
	...
	simplesys = structural_simplify(fol_separate) # removes one equation!
	
	# structural_simplify is "the god of transforms"
...
# ╔═╡ 9361030b-7f95-45d5-80dd-444e2eb5a434
md"""
Let's say you want to build a model with a system of subsystems.
"""

# ╔═╡ c28f8388-1ef8-4bc4-a755-fcea531c65a3
let
	# 25:20
	# https://youtu.be/HEVOgSLBzWA?t=1520
	
	
end

# ╔═╡ 0aed9860-50cc-44ce-9c6d-273ca681bd9d


# ╔═╡ 763eb66a-10fd-489e-b798-595f35beb265
let
	# 26:20
	# https://youtu.be/HEVOgSLBzWA?t=1580
end

# ╔═╡ bbd9edef-2536-4f42-9f61-c44c87b40f23


# ╔═╡ 9d0b2630-7c6d-43fb-885b-2ca2d56d0633
let
	# 27:12
	# https://youtu.be/HEVOgSLBzWA?t=1632
end

# ╔═╡ ecf744d5-f14f-46fd-a7c5-4895fc282d3a
md"""
## Automated ML Acceleration
Note: JuliaSim is a licensed cloud product of JuliaComputing.
"""

# ╔═╡ d563a84b-be90-44fc-b451-ef6853a2cad3
let
	# 28:10
	# https://youtu.be/HEVOgSLBzWA?t=1690
	
	
end

# ╔═╡ aba59238-0da7-4e8b-901c-11024ee45c2c
md"""
## Moment closures for free!
"""

# ╔═╡ deefaf33-7ce8-4789-a6ec-d1559152d37b
let
	# 29:05
	# https://youtu.be/HEVOgSLBzWA?t=1745
	# example source:
	# https://juliahub.com/docs/Catalyst/QGu8a/5.0.5/tutorials/using_catalyst/#Stochastic-Simulation-Algorithms-(SSAs)-for-Stochastic-Chemical-Kinetics
...
# ╔═╡ b9dd7496-547c-45f4-846b-f6d9be1ba117
let
	# 29:47
	# https://youtu.be/HEVOgSLBzWA?t=1787
	...
	# using MomentClosure ]add https://github.com/augustinas1/MomentClosure.jl
...
# ╔═╡ 0fa106bc-8a36-4f21-b7ed-d01224f5d5bd
md"""
## Nonlinear systems
"""

# ╔═╡ 036a2fee-ca7b-47ab-abe9-6b98b3685f9d
let
	# NOT WORKING
	# 30:20
	# https://youtu.be/HEVOgSLBzWA?t=1820
...
# ╔═╡ caa4b75c-eeeb-4f1f-8718-cdaac98cabf0


# ╔═╡ 4281573a-22db-44a4-87e7-1649c5386ca8
let
	# 30:53
	# https://youtu.be/HEVOgSLBzWA?t=1853
	...
	simple_sys = structural_simplify(sys) # simplifies into a single equation for u5
	equations(simple_sys)
end

# ╔═╡ 3faf30d0-2633-49f1-9dc5-f7d576241169
md"""
# MTK's General PDE Specifications
"""

# ╔═╡ b2660496-4ea5-480e-bc4f-6ab645439237


# ╔═╡ 9cb96f67-163e-413b-a4ed-1234411a8b3c


# ╔═╡ 21ef68ed-9063-49de-9ac1-b8c9bd3bc247
md"""
The Yingbo Show
"""

# ╔═╡ efca691f-f196-455c-a92a-9d6c9857e087
let
	@variables x y
	
	# Using Symbolics.jl, take the derivative of an expression wrt x.
	Symbolics.derivative(x^2 + y^2,x)
	
	# Now, let's get the gradient of the function.
	Symbolics.gradient(x^2 + y^2,[x,y])
	
	# Since we have an Rn to Rn function, we can get a Jacobian matrix.
	Symbolics.jacobian([x^2 + y^2; y^2], [x,y])
	
	# Substitution of variables.
	substitute(sin(x)^2 + 2 + cos(x)^2, Dict(x=>y^2))
	
	# Substitution of numeric values.
	substitute(sin(x)^2 + 2 + cos(x)^2, Dict(x=>1))
	
	#
	simplify(sin(x)^2 + 2 + cos(x)^2)
	
	# Simplification during term construction
	2x-x
	...
	# sum(xs) # "lazy" representation ... ?
	sum(collect(xs))
	
end

# ╔═╡ cf946ff7-5cf7-49c7-af48-44fb05be9332
md"""
A more realistic example with the Rosenbrock function.
"""

# ╔═╡ a7b9ce73-e4ac-4bc0-81f8-248dc2974dbb
let
	N = 100
	@variables xs[1:N]
	...
	# Compute the gradient on the Rosenbrock function
	xs = collect(xs)
	rxs = rosenbrock(xs)
	grad = Symbolics.gradient(rxs, xs)
	
	# In optimization algorithms, the gradient is often used, such as with gradient descent or quasi-Newton. Then, if you are a full Newton solver or a Newton trust-region solver, you also need the Hessian of the function. The Hessian is just the Jacobian with respect to the gradient.
	
	hes = Symbolics.jacobian(grad, xs) 
	
	# It is mainlly 0 (wasteful to compute derivatives that are mostly 0).
	
	# We can take advantage of the sparsity by using the sparse Jacobian function.
	
	hes_sparse = Symbolics.sparsejacobian(grad,xs) # symbolically-valued sparse matrix
	
	# If you are only interested in the sparsity pattern but not the exact Hessian values, then you can use the Jacobian_sparsity function.
	
	hes_sparsity = Symbolics.jacobian_sparsity(grad,xs) # Boolean sparse matrix.
	
	# Since hes_sparse is symbolic, we have to convert it so Plots knows what to do. This can be done by broadcasting.
	spy(.!iszero.(hes_sparse)) # Broadcast negation and iszero
	
	# Just plotting the hes_sparsity will work too.
	spy(hes_sparsity)
...
# ╔═╡ b2e6aadb-e605-4305-8377-26dded88f08b


# ╔═╡ 20e72e8e-fb87-4010-b52c-c9064c9f5369


# ╔═╡ 8e9de782-e4bd-49d9-8c62-85c2704d6045
let
	using NeuralPDE, Flux, GalacticOptim, DiffEqFlux
...
# ╔═╡ e940fb7c-d632-4a50-bee3-e70f1c3d4fc8
let
	# using ModelingToolkit
	import ModelingToolkit: Interval, infimum, supremum
...
git-tree-sha1 = "7079b4759f07fd27e621fc480a36a2fb1e43e0d5"
...
[[Catalyst]]
deps = ["AbstractAlgebra", "DataStructures", "DocStringExtensions", "Latexify", "MacroTools", "ModelingToolkit", "Parameters", "Reexport", "Requires", "Symbolics"]
git-tree-sha1 = "5d6b8d1b0c241dd3fe0399df1a115cb6e23e8b39"
...
git-tree-sha1 = "8fb163273547563ee7a0906b880b721646bf0f86"
uuid = "8a292aeb-7a57-582c-b821-06e4c11590b1"
version = "2.2.0"

[[Cuba_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "bda7a143df7c9160f9d7ea9e494f916718191583"
...
[[Cubature_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "99dfeb3678a39bcc50890ff4fe1cee72a51e9180"
...
git-tree-sha1 = "0d2f7b7981a3762abc5701652a4b1e3dd313c631"
...
git-tree-sha1 = "10f12aa638804a505fe5cb94fba697d939cb1473"
...
[[DiffEqFlux]]
deps = ["Adapt", "Cassette", "ConsoleProgressMonitor", "DataInterpolations", "DiffEqBase", "DiffEqSensitivity", "DiffResults", "Distributions", "DistributionsAD", "Flux", "ForwardDiff", "GalacticOptim", "LinearAlgebra", "Logging", "LoggingExtras", "Optim", "Printf", "ProgressLogging", "RecursiveArrayTools", "Reexport", "Requires", "StaticArrays", "TerminalLoggers", "Zygote", "ZygoteRules"]
git-tree-sha1 = "e0856bbd6fa80552d76e70478c17fa345749ee85"
...
git-tree-sha1 = "99542a0279679212d7f22267cad78e714c7b6097"
...
git-tree-sha1 = "de46f37ecd8888f820e9a399e4b7b696e95c92ec"
uuid = "7da242da-08ed-463a-9acd-ee780be4f1d9"
version = "0.6.5"

[[Enzyme_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "886fb83c046fabaf2574c833fec0647427043a75"
...
git-tree-sha1 = "32d24c92578052b107f4fac4081c5af34e64eaef"
...
git-tree-sha1 = "487fd96cc26bbadbab9819259b64ed78c1b26511"
...
git-tree-sha1 = "11ee879a9ee1a12013cb41b8d974f9bd762c3d20"
...
[[Quadrature]]
deps = ["ChainRulesCore", "CommonSolve", "DiffEqBase", "Distributions", "ForwardDiff", "HCubature", "LinearAlgebra", "MonteCarloIntegration", "QuadGK", "Reexport", "Requires", "ReverseDiff", "Zygote", "ZygoteRules"]
git-tree-sha1 = "e7f1665d4354bf431b5e530add0bb10c7076c1b7"
...
[[Trapz]]
git-tree-sha1 = "aff96deaaa38e3b52a5af1dd3da376d49bf6ec9a"
